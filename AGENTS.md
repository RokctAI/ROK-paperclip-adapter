# Rok Paperclip Adapter — Development Guide

## Overview

This is a Paperclip adapter that runs Rok Agent as a managed employee.
...
│   ├── execute.ts        # Core execution (spawn rok CLI)
│   └── test.ts           # Environment checks (CLI, Python, API keys)
├── ui/
│   ├── index.ts          # Re-exports
│   ├── parse-stdout.ts   # Rok stdout → TranscriptEntry[]
...
- `execute(ctx)` — spawns `rok chat -q "..."`, returns `AdapterExecutionResult`
...
4. Create an agent with `adapterType: "rok_local"`
