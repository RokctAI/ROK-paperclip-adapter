/**
 * Detect the current model and provider from the user's Rok config.
 *
 * Reads ~/.rok/config.yaml and extracts the default model,
 * provider, base_url, and api_mode settings.
 *
 * Also provides provider resolution logic that merges explicit config,
 * Rok config detection, and model-name prefix inference.
 */
...
/**
 * Read the Rok config file and extract the default model config.
 */
export async function detectModel(
  configPath?: string,
): Promise<DetectedModel | null> {
  const filePath = configPath ?? join(homedir(), ".rok", "config.yaml");
...
export function resolveProvider(options: {
  ...
  /** Provider detected from Rok config file */
  detectedProvider?: string;
  /** Model name from Rok config file (to check consistency) */
  ...
  // 2. Provider from Rok config file — but ONLY if the config model matches
  //    the requested model. Otherwise the config provider is for a different model
  ...
  if (
    ...
    return { provider: detectedProvider, resolvedFrom: "rokConfig" };
  }
