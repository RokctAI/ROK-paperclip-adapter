/**
 * Server-side execution logic for the Rok Agent adapter.
 *
 * Spawns `rok chat -q "..." -Q` as a child process, streams output,
 * and returns structured results to Paperclip.
 *
 * Verified CLI flags (rok chat):
...
function buildPrompt(
  ...
  const agentName = ctx.agent?.name || "Rok Agent";
...
/** Regex to extract session ID from Rok quiet-mode output: "session_id: <id>" */
const SESSION_ID_REGEX = /^session_id:\s*(\S+)/m;

/** Regex for legacy session output format */
const SESSION_ID_REGEX_LEGACY = /session[_ ](?:id|saved)[:\s]+([a-zA-Z0-9_-]+)/i;

/** Regex to extract token usage from Rok output. */
const TOKEN_USAGE_REGEX =
  /tokens?[:\s]+(\d+)\s*(?:input|in)\b.*?(\d+)\s*(?:output|out)\b/i;

/** Regex to extract cost from Rok output. */
...
/** Strip noise lines from a Rok response (tool output, system messages, etc.) */
function cleanResponse(raw: string): string {
  return raw
    ...
    .filter((line) => {
      ...
      if (t.startsWith("[tool]") || t.startsWith("[rok]") || t.startsWith("[paperclip]")) return false;
...
function parseRokOutput(stdout: string, stderr: string): ParsedOutput {
  const combined = stdout + "\n" + stderr;
  const result: ParsedOutput = {};

  // In quiet mode, Rok outputs:
  //   <response text>
  //
  //   session_id: <id>
...
export async function execute(
  ...
  const rokCmd = cfgString(config.rokCommand) || ROK_CLI;
  ...
  //   2. Provider from ~/.rok/config.yaml (detected at runtime)
  //   3. Provider inferred from model name prefix
  //   4. "auto" (let Rok decide)
  //
  ...
  // "auto" means Rok will decide on its own — no need to pass it.
  ...
  // Requires rok-agent >= PR #3255 (feat/session-source-tag).
  args.push("--source", "tool");

  // Bypass Rok dangerous-command approval prompts.
  ...
  await ctx.onLog(
    "stdout",
    `[rok] Starting Rok Agent (model=${model}, provider=${resolvedProvider} [${resolvedFrom}], timeout=${timeoutSec}s${maxTurns ? `, max_turns=${maxTurns}` : ""})\n`,
  );
  if (prevSessionId) {
    await ctx.onLog(
      "stdout",
      `[rok] Resuming session: ${prevSessionId}\n`,
    );
  }

  // ── Execute ────────────────────────────────────────────────────────────
  // Rok writes non-error noise to stderr (MCP init, INFO logs, etc).
  ...
  const result = await runChildProcess(ctx.runId, rokCmd, args, {
    cwd,
    env,
    timeoutSec,
    graceSec,
  ...
  // ── Parse output ───────────────────────────────────────────────────────
  const parsed = parseRokOutput(result.stdout || "", result.stderr || "");

  await ctx.onLog(
    "stdout",
    `[rok] Exit code: ${result.exitCode ?? "null"}, timed out: ${result.timedOut}\n`,
  );
  if (parsed.sessionId) {
    await ctx.onLog("stdout", `[rok] Session: ${parsed.sessionId}\n`);
  }
