/**
 * Environment test for the Rok Agent adapter.
 *
 * Verifies that Rok Agent is installed, accessible, and configured
...
async function checkCliInstalled(
  ...
  try {
    ...
    if (e.code === "ENOENT") {
      return {
        level: "error",
        message: `Rok CLI "${command}" not found in PATH`,
        hint: "Install Rok Agent: pip install rok-agent",
        code: "rok_cli_not_found",
      };
    }
...
async function checkCliVersion(
  ...
  try {
    ...
    if (version) {
      return {
        level: "info",
        message: `Rok Agent version: ${version}`,
        code: "rok_version",
      };
    }
    return {
      level: "warn",
      message: "Could not determine Rok Agent version",
      code: "rok_version_unknown",
    };
  } catch {
    return {
      level: "warn",
      message:
        "Could not determine Rok Agent version (rok --version failed)",
      hint: "Make sure the rok CLI is properly installed and functional",
      code: "rok_version_failed",
    };
  }
}
  ...
  try {
    ...
    if (match) {
      ...
      if (major < 3 || (major === 3 && minor < 10)) {
        return {
          level: "error",
          message: `Python ${version} found — Rok requires Python 3.10+`,
    ...
    return {
      ...
      hint: "Rok Agent requires Python 3.10+. Install it from python.org",
      code: "rok_python_missing",
    };
  }
}
...
function checkModel(
  ...
  if (!model) {
    return {
      level: "info",
      message: "No model specified — Rok will use its configured default model",
      hint: "Set a model explicitly in Paperclip only if you want to override your local Rok configuration.",
      code: "rok_configured_default_model",
    };
  }
  return {
    level: "info",
    message: `Model: ${model}`,
    code: "rok_model_configured",
  };
}
...
function checkApiKeys(
  ...
  if (!hasAnthropic && !hasOpenRouter && !hasOpenAI && !hasZai && !hasKimi && !hasMiniMax) {
    return {
      ...
      hint: "Set API keys in the agent's env secrets or ~/.rok/.env. Rok supports: ANTHROPIC_API_KEY, OPENROUTER_API_KEY, OPENAI_API_KEY, ZAI_API_KEY, KIMI_API_KEY, MINIMAX_API_KEY",
      code: "rok_no_api_keys",
    };
  }
...
async function checkProviderConsistency(
  ...
  // If provider was explicitly set but doesn't match what Rok config says,
  // that's worth flagging.
  if (explicitProvider && detectedConfig?.provider && explicitProvider !== detectedConfig.provider) {
    return {
      level: "warn",
      message: `Provider mismatch: adapterConfig has "${explicitProvider}" but ~/.rok/config.yaml has "${detectedConfig.provider}". Using adapterConfig value.`,
      hint: `Model "${model}" may not work correctly with provider "${explicitProvider}". Consider aligning with your Rok config or removing the explicit provider to use auto-detection.`,
      code: "rok_provider_mismatch",
    };
  }
  ...
  if (resolvedFrom === "auto" && !explicitProvider) {
    return {
      level: "warn",
      message: `Could not determine provider for model "${model}" — will use Rok auto-detection`,
      hint: "Set an explicit provider in the agent config or ensure ~/.rok/config.yaml has a matching provider for this model.",
      code: "rok_provider_unknown",
    };
  }
...
export async function testEnvironment(
  ...
  const command = asString(config.rokCommand) || ROK_CLI;
