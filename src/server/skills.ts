...
function resolveRokHome(config: Record<string, unknown>): string {
  const env =
...
async function scanRokSkills(
  ...
  try {
    ...
    // ~/.rok/skills/ doesn't exist — no skills available
  }
...
async function buildSkillEntry(
  ...
  return {
    key,
    runtimeName: key,
    desired: true, // Rok loads all available skills
    managed: false,
    state: "installed",
    origin: "user_installed",
    originLabel: "Rok skill",
    locationLabel: `~/.rok/skills/${categoryPath}`,
    readOnly: true, // Rok manages its own skills — Paperclip can't toggle them
...
async function buildRokSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  const home = resolveRokHome(config);
  const rokSkillsHome = path.join(home, ".rok", "skills");
  ...
  // 2. Scan Rok's own skills from ~/.rok/skills/
  const rokSkillEntries = await scanRokSkills(rokSkillsHome);
  const rokKeys = new Set(rokSkillEntries.map((e) => e.key));

  // 3. Merge: Paperclip skills first (ephemeral), then Rok skills
  const entries: AdapterSkillEntry[] = [];
  ...
  for (const entry of paperclipEntries) {
    ...
    entries.push({
      ...
      targetPath: null,
      detail: desired
        ? "Will be available on the next run via Rok skill loading."
        : null,
  ...
  // Rok-installed skills (read-only, always loaded)
  for (const entry of rokSkillEntries) {
  ...
  for (const desiredSkill of desiredSkills) {
    if (availableByKey.has(desiredSkill) || rokKeys.has(desiredSkill)) continue;
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in Paperclip or Rok skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in Paperclip or ~/.rok/skills/.",
    });
  }
...
export async function listRokSkills(
  ctx: AdapterSkillContext,
): Promise<AdapterSkillSnapshot> {
  return buildRokSkillSnapshot(ctx.config);
}

export async function syncRokSkills(
  ctx: AdapterSkillContext,
  _desiredSkills: string[],
): Promise<AdapterSkillSnapshot> {
  // Rok manages its own skill loading — sync is a no-op.
  // Return the current snapshot so the UI stays in sync.
  return buildRokSkillSnapshot(ctx.config);
}

export function resolveRokDesiredSkillNames(
