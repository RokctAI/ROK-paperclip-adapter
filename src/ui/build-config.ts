...
/**
 * Build a Rok Agent adapter config from the Paperclip UI form values.
 */
export function buildRokConfig(
  ...
  // timeoutSec: wall-clock kill timeout for the rok child process.
  // maxTurnsPerRun: maps to Rok's --max-turns (agent tool-calling iterations).
  ...
  // Custom rok binary path
  if (v.command) {
    ac.rokCommand = v.command;
  }
