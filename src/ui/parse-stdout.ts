/**
 * Parse Rok Agent stdout into TranscriptEntry objects for the Paperclip UI.
 *
 * Rok CLI quiet-mode output patterns:
 *   Assistant:  "  ┊ 💬 {text}"
...
function parseToolCompletionLine(
  ...
  // Map Rok verbs to readable tool names
...
function syntheticToolUseId(): string {
  return `rok-tool-${++toolCallCounter}`;
}
...
/**
 * Parse a single line of Rok stdout into transcript entries.
 *
...
export function parseRokStdoutLine(
  ...
  if (trimmed.startsWith("[rok]") || trimmed.startsWith("[paperclip]")) {
