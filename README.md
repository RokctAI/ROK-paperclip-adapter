# Paperclip Adapter for Rok Agent

A [Paperclip](https://paperclip.ing) adapter that lets you run [Rok Agent](https://github.com/RokctAI/rok-agent) as a managed employee in a Paperclip company.

Rok Agent is a full-featured AI agent by [Nous Research](https://rokct.ai) with 30+ native tools, persistent memory, session persistence, 80+ skills, MCP support, and multi-provider model access.
...
- **Skills integration** — Scans both Paperclip-managed and Rok-native skills (`~/.rok/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Rok stdout is parsed into typed `TranscriptEntry` objects so Paperclip renders proper tool cards with status icons and expand/collapse
- **Rich post-processing** — Converts Rok ASCII banners, setext headings, and `+--+` table borders into clean GFM markdown
- **Comment-driven wakes** — Agents wake to respond to issue comments, not just task assignments
- **Auto model detection** — Reads `~/.rok/config.yaml` to pre-populate the UI with the user's configured model
...
### Rok Agent Capabilities

| Feature | Claude Code | Codex | Rok Agent |
|---------|------------|-------|-------------|
...
```bash
npm install rok-paperclip-adapter
```

### Prerequisites

- [Rok Agent](https://github.com/RokctAI/rok-agent) installed (`pip install rok-agent`)
- Python 3.10+
...
```typescript
import * as rokLocal from "rok-paperclip-adapter";
import {
  execute,
  testEnvironment,
  detectModel,
  ...
  syncSkills,
  sessionCodec,
} from "rok-paperclip-adapter/server";

registry.set("rok_local", {
  ...rokLocal,
  execute,
  testEnvironment,
  detectModel,
...
### 2. Create a Rok agent in Paperclip

In the Paperclip UI or via API, create an agent with adapter type `rok_local`:

```json
{
  "name": "Rok Engineer",
  "adapterType": "rok_local",
  "adapterConfig": {
...
Create issues in Paperclip and assign them to your Rok agent. On each heartbeat, Rok will:
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `rokCommand` | string | `rok` | Custom CLI binary path |
...
│                  │               │  MCP Client      │
│  Skill Sync      │◀──snapshot────│  ~/.rok/skills│
│  Org Chart       │               │                  │
└──────────────────┘               └──────────────────┘
```

The adapter spawns Rok Agent's CLI in single-query mode (`-q`). Rok
...
3. **Post-processes** Rok ASCII formatting (banners, setext headings, table borders) into clean GFM markdown
...
Session persistence works via Rok's `--resume` flag — each run picks
...
- **Rok-native skills** — from `~/.rok/skills/`, read-only, always loaded
...
```bash
git clone https://github.com/RokctAI/rok-paperclip-adapter
cd rok-paperclip-adapter
npm install
npm run build
```
...
- [Rok Agent](https://github.com/RokctAI/rok-agent) — The AI agent this adapter runs
- [Paperclip](https://github.com/paperclipai/paperclip) — The orchestration platform
- [Nous Research](https://rokct.ai) — The team behind Rok
